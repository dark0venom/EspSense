...
/* 64 bit ELF emulation code for elf_x86_64_fbsd
...
#define TARGET_IS_elf_x86_64_fbsd
...
static void gldelf_x86_64_fbsd_before_parse (void);
static void gldelf_x86_64_fbsd_after_open (void);
static void gldelf_x86_64_fbsd_before_allocation (void);
static bfd_boolean gldelf_x86_64_fbsd_place_orphan (asection *s);
static void gldelf_x86_64_fbsd_finish (void);


static void
gldelf_x86_64_fbsd_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldelf_x86_64_fbsd_before_parse (void)
{
...
static bfd_boolean
gldelf_x86_64_fbsd_load_symbols (lang_input_statement_type *entry)
{
  int class = 0;
...
static void
gldelf_x86_64_fbsd_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf_x86_64_fbsd_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf_x86_64_fbsd_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needed != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf_x86_64_fbsd_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf_x86_64_fbsd_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf_x86_64_fbsd_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf_x86_64_fbsd_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf_x86_64_fbsd_try_needed (&needed, force))
	return TRUE;
...
static char *
gldelf_x86_64_fbsd_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
static bfd_boolean
gldelf_x86_64_fbsd_check_ld_elf_hints (const struct bfd_link_needed_list *l,
					 int force)
{
	  ...
	  if (fread (&hdr, 1, sizeof (hdr), f) == sizeof (hdr)
	      ...
	      if (fseek (f, hdr.strtab + hdr.dirlist, SEEK_SET) != -1)
		{
		  ...
		  if (fread (b, 1, hdr.dirlistlen + 1, f) ==
		      hdr.dirlistlen + 1)
		    ld_elf_hints = gldelf_x86_64_fbsd_add_sysroot (b);
  ...
  needed.by = l->by;
  needed.name = l->name;
  return gldelf_x86_64_fbsd_search_needed (ld_elf_hints, &needed, force);
}

/* See if an input file matches a DT_NEEDED entry by name.  */

static void
gldelf_x86_64_fbsd_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf_x86_64_fbsd_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf_x86_64_fbsd_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf_x86_64_fbsd_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf_x86_64_fbsd_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf_x86_64_fbsd_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  if (gldelf_x86_64_fbsd_search_needed (command_line.rpath,
						  &n, force))
	    break;
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (gldelf_x86_64_fbsd_search_needed (lib_path, &n,
						      force))
		break;
	    }
	  lib_path = (const char *) getenv ("LD_LIBRARY_PATH");
	  if (gldelf_x86_64_fbsd_search_needed (lib_path, &n, force))
	    break;
	  found = 0;
	  rp = bfd_elf_get_runpath_list (output_bfd, &link_info);
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      char *tmpname = gldelf_x86_64_fbsd_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldelf_x86_64_fbsd_search_needed (tmpname,
							      &n,
							      force));
	      free (tmpname);
	    }
	  ...
	  if (gldelf_x86_64_fbsd_check_ld_elf_hints (l, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf_x86_64_fbsd_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf_x86_64_fbsd_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf_x86_64_fbsd_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf_x86_64_fbsd_find_exp_assignment (exp->binary.lhs);
      gldelf_x86_64_fbsd_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf_x86_64_fbsd_find_exp_assignment (exp->trinary.cond);
      gldelf_x86_64_fbsd_find_exp_assignment (exp->trinary.lhs);
      gldelf_x86_64_fbsd_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf_x86_64_fbsd_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf_x86_64_fbsd_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf_x86_64_fbsd_find_exp_assignment (s->assignment_statement.exp);
}
...
static void
gldelf_x86_64_fbsd_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  ...
  lang_for_each_statement (gldelf_x86_64_fbsd_find_statement_assignment);
  ...
  if (sinterp != NULL)
    {
      sinterp->contents = (unsigned char *) "/usr/libexec/ld-elf.so.1";
...
static bfd_boolean
gldelf_x86_64_fbsd_open_dynamic_archive
...
static bfd_boolean
gldelf_x86_64_fbsd_place_orphan (asection *s)
{
...
static void
gldelf_x86_64_fbsd_finish (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (output_bfd, &link_info);

  gldelf_x86_64_fbsd_map_segments (need_layout);
  finish_default ();
}

static char *
gldelf_x86_64_fbsd_get_script (int *isfile)
{
  *isfile = 0;
...
static void
gldelf_x86_64_fbsd_add_options
...
static bfd_boolean
gldelf_x86_64_fbsd_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldelf_x86_64_fbsd_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf_x86_64_fbsd_emulation =
{
  gldelf_x86_64_fbsd_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldelf_x86_64_fbsd_after_open,
  after_allocation_default,
  set_output_arch_default,
  ldemul_default_target,
  gldelf_x86_64_fbsd_before_allocation,
  gldelf_x86_64_fbsd_get_script,
  "elf_x86_64_fbsd",
  "elf64-x86-64-freebsd",
  gldelf_x86_64_fbsd_finish,
  NULL,
  gldelf_x86_64_fbsd_open_dynamic_archive,
  gldelf_x86_64_fbsd_place_orphan,
  NULL,
  NULL,
  gldelf_x86_64_fbsd_add_options,
  gldelf_x86_64_fbsd_handle_option,
  NULL,
  gldelf_x86_64_fbsd_list_options,
  gldelf_x86_64_fbsd_load_symbols,
  NULL,
  NULL
};
