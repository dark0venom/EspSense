...
ippool_yyloop:
    ...
    {
        ippool_yychar = YYLEX;
        if (ippool_yychar < 0) ippool_yychar = YYEOF;
#if YYDEBUG
        if (ippool_yydebug)
        {
            if ((ippool_yys = ippool_yyname[YYTRANSLATE(ippool_yychar)]) == NULL) ippool_yys = ippool_yyname[YYUNDFTOKEN];
            printf("%sdebug: state %d, reading %d (%s)\n",
                    YYPREFIX, ippool_yystate, ippool_yychar, ippool_yys);
        }
#endif
    }
    if (((ippool_yyn = ippool_yysindex[ippool_yystate]) != 0) && (ippool_yyn += ippool_yychar) >= 0 &&
            ippool_yyn <= YYTABLESIZE && ippool_yycheck[ippool_yyn] == (YYINT) ippool_yychar)
    {
#if YYDEBUG
        if (ippool_yydebug)
...
#endif
        if (ippool_yystack.s_mark >= ippool_yystack.s_last && ippool_yygrowstack(&ippool_yystack) == YYENOMEM) goto ippool_yyoverflow;
    ...
    if (((ippool_yyn = ippool_yyrindex[ippool_yystate]) != 0) && (ippool_yyn += ippool_yychar) >= 0 &&
            ippool_yyn <= YYTABLESIZE && ippool_yycheck[ippool_yyn] == (YYINT) ippool_yychar)
    {
        ippool_yyn = ippool_yytable[ippool_yyn];
        goto ippool_yyreduce;
    }
    if (ippool_yyerrflag != 0) goto ippool_yyinrecovery;

    YYERROR_CALL("syntax error");

    goto ippool_yyerrlab; /* redundant goto avoids 'unused label' warning */
ippool_yyerrlab:
    ++ippool_yynerrs;
...
ippool_yyinrecovery:
    if (ippool_yyerrflag < 3)
    {
        ...
        for (;;)
        {
            if (((ippool_yyn = ippool_yysindex[*ippool_yystack.s_mark]) != 0) && (ippool_yyn += YYERRCODE) >= 0 &&
                    ippool_yyn <= YYTABLESIZE && ippool_yycheck[ippool_yyn] == (YYINT) YYERRCODE)
            {
#if YYDEBUG
                if (ippool_yydebug)
...
#line 1531 "ippool.tab.c"
 ...
 state %d\n", YYPREFIX, YYFINAL);
    ...
    if (((ippool_yyn = ippool_yygindex[ippool_yym]) != 0) && (ippool_yyn += ippool_yystate) >= 0 &&
            ippool_yyn <= YYTABLESIZE && ippool_yycheck[ippool_yyn] == (YYINT) ippool_yystate)
        ippool_yystate = ippool_yytable[ippool_yyn];
    else
