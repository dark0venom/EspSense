...
add_wrap(int argc, char **argv)
{
    ...
    int help_flag = 0;
    opt.principal_string = (char *)(unsigned long)"";
    opt.kvno_integer = -1;
...
purge_wrap(int argc, char **argv)
{
    ...
    int help_flag = 0;
    opt.age_string = (char *)(unsigned long)"1 week";
...
{
    struct rename_options opt;
    int ret;
    int optidx = 0;
    struct getargs args[] = {
        { "delete", 0, arg_negative_flag, NULL, "don't delete orignal entry", NULL },
        { "help", 'h', arg_flag, NULL, NULL, NULL }
    };
    int help_flag = 0;
    opt.delete_flag = 1;
    args[0].value = &opt.delete_flag;
    args[1].value = &help_flag;
    if(getarg(args, 2, argc, argv, &optidx))
        goto usage;
    if(argc - optidx != 2) {
    ...
    if(help_flag)
        goto usage;
    ret = kt_rename(&opt, argc - optidx, argv + optidx);
    return ret;
usage:
    arg_printusage (args, 2, "rename", "from to");
    return 0;
}
...
destroy_wrap(int argc, char **argv)
{
    ...
    if(help_flag)
        goto usage;
    ret = kt_destroy(NULL, argc - optidx, argv + optidx);
    return ret;
usage:
    arg_printusage (args, 1, "destroy", "");
    return 0;
}
...
SL_cmd commands[] = {
        ...
        { "destroy", destroy_wrap, "destroy", "Destroy (remove) the keytab." },
